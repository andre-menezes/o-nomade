  ...
  <article
    ...
    <div class="flex flex-col shrink justify-between px-4 py-4">
      <div class="flex flex-col items-center">
        <p class="pb-2 text-lg text-center font-bold">
          <i :class="getIconClass('hotel')"></i>
          {{ hotel.name }}
        </p>
        <p class="py-2">
          <i v-for="n in hotel.rating" :class="getIconClass('star')" class="w-4 text-yellow-300 drop-shadow"></i>
        </p>
        ...
        <button type="button" @click="moreDetails(hotel.id)"
          ...
          <i :class="getIconClass('details')"></i>
          Mais detalhes
        </button>
      </div>
    </div>
